...
function adjectiveQuestion(item: Vocabulary): VocabularyTrainingQuestion | null {
  return getVocabularyWritingQuestion(item);
}
...
function SpecialtyVocabularyTrainer({ config, onNavigate, settings }: TrainerProps & { config: TrainerConfig }) {
  ...
  const eligibleItems = useMemo(
    () => items.filter((item) => Boolean(config.makeQuestion(item, items))),
    [config, items]
  );
  ...
  const setupQuestion = (list: Vocabulary[], index: number) => {
    ...
    setQuestion(prepareQuestion(config.makeQuestion(item, items), item));
  ...
  useEffect(() => {
    if (!loading && items.length) {
      const available = items.filter((item) => Boolean(config.makeQuestion(item, items)));
            ...
            {question.options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 rounded-xl border text-sm font-medium transition-all flex items-center justify-between cursor-pointer ${btnClass}`}
                >
                  <span dir={question.answerLang === "ar" ? "rtl" : "ltr"} lang={question.answerLang} className="w-full">
                    {option}
                  </span>
...
const configs: Record<TrainerKind, TrainerConfig> = {
  ...
  adjective: {
    ...
    makeQuestion: adjectiveQuestion,
  },
  phrase: {
    kind: "phrase",
